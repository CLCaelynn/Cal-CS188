...
def constructBayesNet(gameState):
    ...
    edges.extend([(X_POS_VAR, FOOD_HOUSE_VAR), (X_POS_VAR, GHOST_HOUSE_VAR), (Y_POS_VAR, FOOD_HOUSE_VAR), (Y_POS_VAR, GHOST_HOUSE_VAR)])
    for obsVar in obsVars:
        edges.extend([(FOOD_HOUSE_VAR, obsVar), (GHOST_HOUSE_VAR, obsVar)])
...
def fillObsCPT(bayesNet, gameState):
    ...
    housePositions = {BOTTOM_LEFT_VAL: bottomLeftPos, TOP_LEFT_VAL: topLeftPos, BOTTOM_RIGHT_VAL: bottomRightPos, TOP_RIGHT_VAL: topRightPos}
    # Iterate over possible house positions.
    for housePos in gameState.getPossibleHouses():
        # Iterate over possible wall positions.
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            obsFactor = bn.Factor([obsVar], HOUSE_VARS, bayesNet.variableDomainsDict())
            # Iterate over assignments as in fillHouseCPT above.
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                ...
                # Food House, or both housess
                if housePos == housePositions[foodHousePos]:
                    probability = PROB_FOOD_RED
                    if assignment[obsVar] == RED_OBS_VAL:
                        obsFactor.setProbability(assignment, probability)
                    if assignment[obsVar] == BLUE_OBS_VAL:
                        obsFactor.setProbability(assignment, 1 - probability)
                # Ghost house
                elif housePos == housePositions[ghostHousePos] and ghostHousePos != foodHousePos:
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    currLocation, currProb = '', 0
    for assignment in foodHouseFactor.getAllPossibleAssignmentDicts():
        assProb = foodHouseFactor.getProbability(assignment)
        if assProb > currProb:
            currLocation = assignment[FOOD_HOUSE_VAR]
            currProb = assProb
    return {FOOD_HOUSE_VAR: currLocation}
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        "*** YOUR CODE HERE ***"
        housesFactor = inference.inferenceByVariableElimination(self.bayesNet, [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], evidence, eliminationOrder)
        for assignment in housesFactor.getAllPossibleAssignmentDicts():
            assProb = housesFactor.getProbability(assignment)
            if assignment[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL:
                leftExpectedValue += (assProb * WON_GAME_REWARD) + ((1 - assProb) * GHOST_COLLISION_REWARD)
            if assignment[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_LEFT_VAL:
                rightExpectedValue += (assProb * WON_GAME_REWARD) + ((1 - assProb) * GHOST_COLLISION_REWARD)
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        updatedEvidence = {}
        for ev in evidence:
            updatedEvidence[ev] = evidence[ev]
        for obsProb, observations in self.getExplorationProbsAndOutcomes(evidence):
            for obs in observations:
                updatedEvidence[obs] = observations[obs]
            optimalActionValue = max(self.computeEnterValues(updatedEvidence, enterEliminationOrder))
            expectedValue += obsProb * optimalActionValue
